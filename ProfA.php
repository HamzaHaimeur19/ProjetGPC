...
<html lang="en">
  ...
  <nav class="navbar navbar-dark bg-dark" style="background-color:yellow ;">
        ...
        //recuperation des informations de l'utilisateur à l'aide de la boucle foreach
         foreach ($query->fetchAll(PDO::FETCH_ASSOC) as $row) { ?>
          <a class="navbar-brand" href="#">Vous pouvez bien consulter les professeurs appartenant à ce departement<?php echo ' '.$row['nom'].' '.$row['prenom']; }  ?></a>
          ...
          <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            <div class="navbar-nav">
              ...
              <a class="nav-link" href="#">Gérer les cours</a>
            </div>
          </div>
        </div>
      </nav>
  ...
  <tbody>
  <?php 
     $stm=$pdo->prepare("select p.Id_prof, p.nom, p.prenom, p.cin, p.adresse, p.telephone, p.email, p.date_recrutement, d.nom_department from department d, professeur p  where p.id_department=d.id_department and d.id_department=:userid");
     $idm=$_GET["id"];
