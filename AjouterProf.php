...
<nav class="navbar navbar-dark bg-dark" style="background-color:yellow ;">
        ...
        //recuperation des informations de l'utilisateur à l'aide de la boucle foreach
         foreach ($query->fetchAll(PDO::FETCH_ASSOC) as $row) { ?>
          <a class="navbar-brand" href="#">Vous pouvez bien ajouter un professeur à partir de ce formulaire<?php echo ' '.$row['nom'].' '.$row['prenom']; }  ?></a>
      ...
      <br><br><br>
	<div style="height:470px; margin-top:120px;" id="container">
      ...
      <?php
       ...
       $data = $stm->fetchAll();
       ?>
		<form action="Aprof.php" method="post">
      ...
      <input type="text" id="date_recrutement" name="date_recrutement" placeholder="date r(aaaa-mm-jj)" required>
      <select id="id_department" name="id_department" >
        <option selected disabled value="choisir">Choisir Dept</option>
      <?php
       foreach ($data as $row){ ?>
	...
	if(isset($_SESSION["ErrProf"])){ ?>
<div class="alert">
  <span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span> 
  <strong><?php echo($_SESSION["ErrProf"]); ?></strong>
</div>
<?php
    unset($_SESSION["ErrProf"]);
  }
    ?>
</body>
</html>